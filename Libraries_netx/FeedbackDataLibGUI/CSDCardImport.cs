                ...
                if (benoughbytesreceived)
                {
                    ...
                    if (CInsightDataEnDecoder.Parse4Byte(RS232inBytes) is CDataIn di)
                    {
                        ...
                        else
                        {
                            ...
                            if (DataIn.HWcn < ModuleInfo.Count)
                            {
                                if (ModuleInfo[DataIn.HWcn].IsModuleActive())
                                {
                                    if (DataIn.SWcn < ModuleInfo[DataIn.HWcn].NumSWChannels)
                                    {
                                        if (ModuleInfo[DataIn.HWcn].SWChannels[DataIn.SWcn].SaveChannel)
                                        {
                                            bpacketvalid = true;
                                            isDataByte = true;
                                            ...
                                            if (DataIn.SyncFlag == 1)
                                            {
                                                ...
                                                {
                                                    DataReceiver.Connection.Device.ModuleInfos[DataIn.HWcn].SWChannels[DataIn.SWcn].SWChan_Started = Recording_Start_Time_from_File;
                                                }

                                                ModuleInfo[DataIn.HWcn].SWChannels[DataIn.SWcn].SynPackagesreceived = cntSyncPackages;
                                            }
                                            ...
                                            if (isDataByte)
                                            {
                                                ...
                                                try
                                                {
                                                    ...
                                                    if (DataReceiver != null && DataReceiver.Connection != null && DataReceiver.Connection.Device != null)
                                                        DataReceiver.Connection.Device.ModuleInfos[DataIn.HWcn].SWChannels[DataIn.SWcn].UpdateTime(ref DataIn);
                                                    if (DataIn.DTRelative == DateTime.MinValue)
                                                    {
