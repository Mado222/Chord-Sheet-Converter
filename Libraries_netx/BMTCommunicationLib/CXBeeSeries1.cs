    ...
    ************************************************************************************/
    ...
    public class CXBeeSeries1(ISerialPort SerialPort) : IDisposable
    {
        ...
        public CBasicAPIResponse? XBGetResponse()
        {
            ...
            if (LocalDevice == null || LocalDevice.APIMode == XBAPIMode.Enabled_w_Escape_Control_Chars)
            {
        ...
        public List<CXBNodeInformation>? XBNodeDiscoverAPI()
        {
            ...
            if (LocalDevice is null || LocalDevice.APIMode == XBAPIMode.Disabled)
            {
                //XBSetLastError(9);
        ...
        public CXBNodeInformation? XBGetConfigurationOfLocalDevice()     //old name: public CXBNodeInformation XBConnectToLocalDevice()
        {
            if (XBCheckCommandMode(true) > 0)
            {
                ...
                string? s = XBGetString(1000);
                if (s != null && LocalDevice is not null)
                {
        ...
        /// <remarks>This method works in API-mode and in AT-Mode</remarks>
        /// <returns>
        /// >1: if register successfully set and modem-status-message (status=return value) was received
        /// 1: register successfully set
        /// -1: an error occurs
        ...
        {
            return SetRegisterOnLocalDevice(
                CXBATCommands.XBAT_SetCommandSequenceCharacter((byte)newCharacter),
        ...
        {
            return SetRegisterOnLocalDevice(
                CXBATCommands.XBAT_SetGuardTimes(newGuardTimes),
                () => _GuardTimes = newGuardTimes
            );
        }

        /// <summary>
        /// Helper method to set a register on the local device based on the command and update action.
        /// </summary>
        /// <param name="ATCommand">The AT command to set the register.</param>
        /// <param name="updateAction">Action to update the local device state upon successful execution.</param>
        /// <returns>Result of the command execution.</returns>
        private int SetRegisterOnLocalDevice(string ATCommand, Action updateAction)
        {
            if (LocalDevice == null)
                throw new InvalidOperationException("LocalDevice cannot be null.");

            int res = LocalDevice.APIMode == XBAPIMode.Enabled ||
                      LocalDevice.APIMode == XBAPIMode.Enabled_w_Escape_Control_Chars
                      ? XBSendSetCommand2LocalDeviceAPIMode(ATCommand)
                      : XBSendSetCommand2LocalDeviceCMDMode(ATCommand);
        ...
        {
            return SetRegisterOnLocalDevice(
                CXBATCommands.XBAT_SetNodeDiscoverTime(newNodeDiscoverTime),
        ...
        public int XBSetAPIMode2LocalDevice(XBAPIMode APIMode)
        {
            ...
            return SetRegisterOnLocalDevice(
                CXBATCommands.XBAT_SetAPIMode(APIMode),
        ...
        public CXBNodeInformation? XBGetConfigurationOfRemoteDevice(ulong EndDeviceSerialNumber)
        {
            ...
            if (LocalDevice != null && LocalDevice.APIMode == XBAPIMode.Disabled)
            {
                //XBSetLastError(9);
                _Error.SetError_LocalDevNotInApi();
            ...
            else if (NodeInfos.Count == 1)
            {
                ...
                if (remoteDevice != null)
                {
                    ...
                    if (def.CompareTo(remoteDevice) != 0)
                    {
                        ...
                        {
                            remoteCommand.ATCommand = CXBATCommands.XBAT_SotfwareReset;
                            XBSendRequest(remoteCommand, 500);
                            Thread.Sleep(1000);
